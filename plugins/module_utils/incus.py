...
class IncusClient(object):
    def __init__(self, url, key_file=None, cert_file=None, debug=False, server_cert_file=None, server_check_hostname=True):
        ...
        :param url: The URL of the Incus server. (e.g. unix:/var/lib/incus/unix.socket or https://127.0.0.1)
        :type url: ``str``
        ...
        if url.startswith('https:'):
            ...
            raise IncusClientException('URL scheme must be unix: or https:')
    ...
    def _send_request(self, method, url, body_json=None, ok_error_codes=None, timeout=None):
        try:
            ...
            raise IncusClientException('cannot connect to the Incus server', err=e)

    def _raise_err_from_json(self, resp_json):
        err_params = {}
        if self.debug:
            err_params['logs'] = self.logs
        raise IncusClientException(self._get_err_from_resp_json(resp_json), **err_params)
...
def default_key_file():
    return os.path.expanduser('~/.config/incus/client.key')


def default_cert_file():
    return os.path.expanduser('~/.config/incus/client.crt')
